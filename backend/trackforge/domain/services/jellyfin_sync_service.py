...
async def sync_jellyfin_library(db: AsyncSession) -> int:
    ...
    for jf_id, album in jellyfin_items.items():
        ...
        if jf_id in existing:
            ...
            # Always preserve manually linked MBIDs over Jellyfin's auto-detected ones
            old_meta = item.metadata_ or {}
            if old_meta.get("manual_mbid"):
                metadata["mbid"] = old_meta["mbid"]
                metadata["manual_mbid"] = True
            elif not metadata["mbid"] and old_meta.get("mbid"):
                metadata["mbid"] = old_meta["mbid"]
